...
# Ensure admin user has proper RBAC permissions for exec/logs operations
Write-Host "Checking RBAC permissions..." -ForegroundColor Cyan
try {
    $binding = kubectl get clusterrolebinding kubernetes-admin 2>$null
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  Creating RBAC binding for kubectl exec/logs operations..." -ForegroundColor Yellow
        kubectl create clusterrolebinding kubernetes-admin --clusterrole=cluster-admin --user=kubernetes 2>$null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  ✅ RBAC permissions configured" -ForegroundColor Green
        } else {
            Write-Host "  ⚠️  RBAC binding creation failed - exec/logs tests may fail" -ForegroundColor Yellow
        }
    } else {
        Write-Host "  ✅ RBAC permissions already configured" -ForegroundColor Green
    }
} catch {
    Write-Host "  ⚠️  Could not verify RBAC permissions" -ForegroundColor Yellow
}
